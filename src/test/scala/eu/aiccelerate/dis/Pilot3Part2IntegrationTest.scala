...
class Pilot3Part2IntegrationTest extends PilotTestSpec {
  ...
  val symptomMappingTask = FhirMappingTask(
    ...
    sourceBinding = Map("source" -> FileSystemSource(path = "symptoms.csv", contentType = "csv"))
  )
  ...
  val neuroObsMappingTask = FhirMappingTask(
    ...
    sourceBinding = Map("source" -> FileSystemSource(path = "neurooncological-observations.csv", contentType = "csv"))
  )
  ...
  val medAdministrationMappingTask = FhirMappingTask(
    ...
    sourceBinding = Map("source" -> FileSystemSource(path = "medication-administrations.csv", contentType = "csv"))
  )
...
//  }
  ...
  "lab results mapping" should "map test data" in {
    //Some semantic tests on generated content
    fhirMappingJobManager.executeMappingTaskAndReturn(mappingJobExecution = FhirMappingJobExecution(mappingTasks = Seq(labResultsMappingTask), job = testJob), mappingJobSourceSettings = dataSourceSettings) map { mappingResults =>
      val results = mappingResults.map(r => {
        r.mappedResource shouldBe defined
        val resource = r.mappedResource.get.parseJson
        resource shouldBe a[Resource]
  ...
  "symptoms mapping" should "map test data" in {
    //Some semantic tests on generated content
    fhirMappingJobManager.executeMappingTaskAndReturn(mappingJobExecution = FhirMappingJobExecution(mappingTasks = Seq(symptomMappingTask), job = testJob), mappingJobSourceSettings = dataSourceSettings) map { mappingResults =>
      val results = mappingResults.map(r => {
        r.mappedResource shouldBe defined
        val resource = r.mappedResource.get.parseJson
        resource shouldBe a[Resource]
  ...
  "neurooncological observations mapping" should "map test data" in {
    //Some semantic tests on generated content
    fhirMappingJobManager.executeMappingTaskAndReturn(mappingJobExecution = FhirMappingJobExecution(mappingTasks = Seq(neuroObsMappingTask), job = testJob), mappingJobSourceSettings = dataSourceSettings) map { mappingResults =>
      val results = mappingResults.map(r => {
        r.mappedResource shouldBe defined
        val resource = r.mappedResource.get.parseJson
        resource shouldBe a[Resource]
  ...
  "medication administration mapping" should "map test data" in {
    //Some semantic tests on generated content
    fhirMappingJobManager.executeMappingTaskAndReturn(mappingJobExecution = FhirMappingJobExecution(mappingTasks = Seq(medAdministrationMappingTask), job = testJob), mappingJobSourceSettings = dataSourceSettings) map { mappingResults =>
      val results = mappingResults.map(r => {
        r.mappedResource shouldBe defined
        val resource = r.mappedResource.get.parseJson
        resource shouldBe a[Resource]
